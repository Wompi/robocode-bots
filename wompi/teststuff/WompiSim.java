...
public class WompiSim
{
	...
	public static void simulate(double att, double d, double maxv)
	{
		...
		x += Math.sin(h = Utils.normalNearAbsoluteAngle(h
				+ limit(Rules.getTurnRateRadians(v), Utils.normalRelativeAngle(att))))
				* (v = limit(maxv, d));
		y += Math.cos(h) * v;
	}
	...
	// velocity function
	public static double nextVeocity(double tv, double d, double maxv)
	{
		if (d == 0) d = Math.signum(tv);
		if (tv * d < 0) d *= 2;
		if (((d = tv + d) * tv) < 0) d /= 2.0;
		return limit(maxv, d);
	}
}
